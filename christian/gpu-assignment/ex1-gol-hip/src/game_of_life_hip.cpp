...
__global__ void cpu_game_of_life_step(int *current_grid, int *next_grid, int n, int m){
    int neighbours;
    int n_i[8], n_j[8];
    unsigned int idx_in = blockDim.x * blockIdx.x + threadIdx.x;
    if( idx_in < n*m)
    {
            unsigned int i = idx_in / n;
            unsigned int j = idx_in % n;
...
int* cpu_game_of_life(const int *initial_state, int n, int m, int nsteps){
    ...
    int *grid,*updated_grid_dev;
    HIP_CHECK_ERROR(hipMalloc(&grid, sizeof(int)*n*m));
    HIP_CHECK_ERROR(hipMalloc(&updated_grid_dev,sizeof(int)*n*m));
    //HIP_CHECK_ERROR(hipMemcpy(grid, initial_state, sizeof(int)*n*m, hipMemcpyHostToDevice));
    ...
    while(current_step != nsteps){
        ...
        game_of_life_step<<<nBlocks,NTHREADS>>>(grid,updated_grid_dev,m,n)
        HIP_CHECK_ERROR(hipGetLastError());
        HIP_CHECK_ERROR(hipDeviceSynchronize());
        HIP_CHECK_ERROR(hipMemcpy(updated_grid, updated_grid_dev, sizeof(int) * n * m, hipMemcpyDeviceToHost));
        HIP_CHECK_ERROR(hipDeviceSynchronize());
        HIP_CHECK_ERROR(hipMemcpy(grid, updated_grid, sizeof(int) * n * m, hipMemcpyHostToDevice));
